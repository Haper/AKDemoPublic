...
export const useLocalStorageValues = (key?: StorageKeysType) => {
  ...
  const handlerRef = useRef<typeof storageUpdated | null>(null);
  ...
  useEffect(() => {
    if (handlerRef.current) {
      window.removeEventListener(DISPATCH_EVENT_STORAGE_UPDATED as 'storage', handlerRef.current);
    }
    window.addEventListener(DISPATCH_EVENT_STORAGE_UPDATED as 'storage', storageUpdated);
    handlerRef.current = storageUpdated;
    return () => {
